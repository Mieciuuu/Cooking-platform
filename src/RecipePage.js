...
const RecipePage = ({ recipeId, onBackButtonClick, loggedIn, userId }) => {
  ...
  const handleRatingChange = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:8080/addRating`, userScore);
      ...
      {recipe && (
        <>
          <tbody id='recipeClass'>
            ...
            <tr>{userId === recipe.a.authorId && (<td className='recipeViewSegment' id='ownRecipeIndicator'>Twój przepis</td>)}<td colSpan={2}><button onClick={onBackButtonClick}>Powrót do wyszukiwania</button></td></tr>
          </tbody>
        </>
      )}
    </div>
